...
if __name__ == '__main__':
    ...
    # 统一定义实验参数（抛弃 main2.py 的传参）
    subject_turn = 1
    is_observe = False           # 你的数据全是想象期，统一定义为 False
    bandpass_filter_range = [0.1, 30] 
    data_time_range = [0, 5]     # 根据你实际切分的时长填写
    network_name = 'EEGNet_LSTM' # 或者你想用的网络
    
    # 开始遍历所有受试者
    for subject_name in subject_list:
        print(f'\n' + '='*60)
        print(f' 开始训练受试者: {subject_name}')
        print('='*60 + '\n')
        
        # 沿用师兄的路径生成逻辑，这样跑出来的结果会自动按人名建文件夹保存
        path_training = get_path(
            path_spec='training', is_file=False, s_name=subject_name, s_turn=subject_turn,
            obs=is_observe, fr=bandpass_filter_range, tr=data_time_range, net=network_name
        )
        path_optimize = get_path(
            path_spec='optimize', is_file=False, s_name=subject_name, s_turn=subject_turn,
            obs=is_observe, fr=bandpass_filter_range, tr=data_time_range, net=network_name
        )
        path_result = get_path(
            path_spec='result', is_file=False, s_name=subject_name, s_turn=subject_turn,
            obs=is_observe, fr=bandpass_filter_range, tr=data_time_range, net=network_name
        )
    ...
    # -------------------------------------------------------------------
        ...
        # region 载入数据集
        print(f'Loading custom dataset for {subject_name}...')

        # ------------------ 核心修改区 2：动态载入纯净 NPY 数据 ------------------
        # 【注意】这里请修改为你存放所有受试者 .npy 文件的真实文件夹路径！
        base_data_dir = r"D:\SUSTech_Lab\PanDeng\OMI Decoding I\src\data"  # 你的数据文件夹路径
        
        # 假设你的文件命名规则是 "受试者名_eeg.npy" 和 "受试者名_label.npy"
        eeg_file_path = os.path.join(base_data_dir, f"{subject_name}_1.npy")
        label_file_path = os.path.join(base_data_dir, f"{subject_name}_label.npy")
        ...
        try:
            ...
            print(f'    ⚠️ 找不到 {subject_name} 的数据文件，跳过该受试者！')
            continue
